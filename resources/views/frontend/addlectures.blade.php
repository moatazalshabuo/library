  ...
  <div class="container" style="margin-top:80px">
    ...
    <div class="row">
        <div class="col-lg-12" data-wow-duration="0.5s" data-wow-delay="0.25s" style="visibility: visible;-webkit-animation-duration: 0.5s; -moz-animation-duration: 0.5s; animation-duration: 0.5s;-webkit-animation-delay: 0.25s; -moz-animation-delay: 0.25s; animation-delay: 0.25s;">
            <form id="contact" action="{{ route('lectures.store') }}" method="post" enctype="multipart/form-data">
              @csrf
              <div class="row">
                <div class="col-lg-8">
                    <div class="col-lg-12 ">
                        ...
                        <div class="row">
                          <div class="col-lg-6">
                            ...
                            <fieldset>
                              ...
                              <input type="name" name="descripe" id="name" value="{{ old('descripe') }}" placeholder="المحاضرة رقم ...." autocomplete="on" required="">
                            </fieldset>
                            @error('descripe')
                            ...
                            <fieldset>
                              <select  name="course_id" class="form-control">
                                ...
                                @foreach ($courses as $item)
                                    <option @selected(old('course_id') == $item->id) value="{{ $item->id }}">{{ $item->course_name }}</option>
                                @endforeach
                              </select>
                            </fieldset>
                            @error('course_id')
                          ...
                          <div class="col-lg-6">
                            <label> المحاضر</label>
                            <fieldset>
                              <input type="text" name="tech" id="email" value="{{ old('tech') }}"  placeholder="المحاضر" required="">
                            </fieldset>
                            @error('tech')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                          ...
                          <div class="col-lg-6">
                            <label> السنة</label>
                            <fieldset>                    
                                  <input type="number" name="year" min="1900" max="2100" value="{{ old('year') }}" placeholder="السنة الدراسية" autocomplete="on">
                                </fieldset>
                                @error('year')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
